...
def on_tick_callback(tick):
    # O trading_bot já alimenta o digit_analyzer internamente
...
def api_reset_password():
    ...
    # Guardar token no utilizador (expira em 1h)
...
def api_trade_hybrid():
    try:
        ...
        if not deriv_client or not deriv_client.authorized:
            return jsonify({'error': 'Não conectado'}), 400
        # Verificar se símbolo atual é R_ para ter dígitos válidos
