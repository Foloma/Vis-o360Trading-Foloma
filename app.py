    ...
    with sessions_lock:
        if user_id not in user_sessions:
            ...
            user_sessions[user_id] = {
                'client':client,'trading_bot':bot,
                'digit_analyzer':analyzer,'payment_system':payment
            }
...
def require_admin(f):
    ...
    def d(*a,**kw):
        ...
        if not user or user.get('role')!='admin': return jsonify({'error':'Acesso restrito ao administrador'}), 403
        return f(*a,**kw)
    return d
    ...
    if 'user_id' in session:
        ...
        if user:
            return jsonify({'authenticated':True,'user':{'id':user['id'],'name':user['name'],'email':user['email'],
                             'role':user.get('role','user'),'has_deriv_token':bool(user.get('deriv_token'))}})
...
def api_login():
    try:
        ...
        if not check_password_hash(user['password'], password): return jsonify({'error':'Senha incorreta'}), 400
        user['last_login'] = time.time(); save_users(users)
        session['user_id'] = user['id']; session['user_name'] = user['name']
        session['user_email'] = user['email']; session['user_role'] = user.get('role','user')
        return jsonify({'status':'ok','user':{'id':user['id'],'name':user['name'],'email':user['email'],
                         'role':session['user_role'],'has_deriv_token':bool(user.get('deriv_token'))}})
    except Exception as e: return jsonify({'error':str(e)}), 500
...
def send_reset_email(to_email, token):
    ...
    msg = MIMEMultipart(); msg['From']=SMTP_USER; msg['To']=to_email
...
def api_reset_password():
    if not SMTP_USER or not SMTP_PASSWORD: return jsonify({'error':'Recuperação não configurada'}), 500
    d=request.json; email=d.get('email','').strip().lower()
    if email not in users: return jsonify({'error':'Email não registado'}), 404
    token=secrets.token_urlsafe(32); users[email]['reset_token']=token
    users[email]['reset_token_expiry']=time.time()+3600; save_users(users)
...
def api_reset_password_confirm():
    ...
    for email,user in users.items():
        if user.get('reset_token')==token and user.get('reset_token_expiry',0)>time.time():
            user['password']=generate_password_hash(pw); user.pop('reset_token',None)
            user.pop('reset_token_expiry',None); save_users(users)
...
def api_connect():
    try:
        ...
        sess=get_user_session(user['id']); client=sess['client']
        client.set_user_token(token); client.connect()
        timeout=time.time()+15
        while not client.authorized and time.time()<timeout: time.sleep(0.3)
        if not client.authorized: return jsonify({'error':'Autorização falhou.'}), 500
        client.subscribe_ticks(symbol); sess['trading_bot'].start(client)
        time.sleep(0.5)
...
def api_status():
    try:
        user_id=session['user_id']; sess=get_user_session(user_id)
        client=sess['client']; bot=sess['trading_bot']; analyzer=sess['digit_analyzer']
        if client:
            bot.balance=client.balance; bot.currency=client.currency
            bot.client=client
            bot._client_connected = client.connected
            bot._client_authorized = client.authorized
        bot_status=bot.get_status()
        analysis=analyzer.get_analysis(); all_digits=analyzer.get_recent_digits()
        digits = {
            'last':analyzer.get_current_digit(),'parity':analyzer.get_current_parity(),
            'stats':analyzer.get_stats(),'analysis':analysis,'recent':all_digits,
            'total':len(all_digits),'ticks_remaining':analyzer.get_ticks_remaining(),
            'digit_counter':analyzer.get_digit_counter(),'ticks_per_digit':analyzer.TICKS_PER_DIGIT,
        }
...
def api_display_digit():
    try:
        d,p,tr = get_user_session(session['user_id'])['digit_analyzer'].get_next_display_digit()
...
def api_symbol_change():
    try:
        ...
        sess=get_user_session(session['user_id']); client=sess['client']
        if client: client.change_symbol(sym); sess['trading_bot'].current_symbol=sym
        return jsonify({'status':'ok','symbol':sym})
    except Exception as e: return jsonify({'error':str(e)}), 500

def credit_affiliate_commission(user_email, amount):
    user = users.get(user_email)
    if not user or not user.get('referral_code'): return
...
def api_trade():
    try:
        ...
        sess=get_user_session(session['user_id']); client=sess['client']; bot=sess['trading_bot']
        if not client or not client.authorized: return jsonify({'error':'Não conectado'}), 400
        ...
        if ok: credit_affiliate_commission(session.get('user_email'), amt); return jsonify({'status':'ok','message':f'Trade {action} enviado','confidence':conf})
...
def api_trade_digit():
    try:
        ...
        sess=get_user_session(session['user_id']); client=sess['client']; analyzer=sess['digit_analyzer']
        if not client or not client.authorized: return jsonify({'error':'Não conectado'}), 400
        if amt<0.35 or amt>100: return jsonify({'error':'Valor inválido'}), 400
...
def api_trade_hybrid():
    try:
        ...
        sess=get_user_session(session['user_id']); client=sess['client']; bot=sess['trading_bot']; analyzer=sess['digit_analyzer']
        if not client or not client.authorized: return jsonify({'error':'Não conectado'}), 400
        if 'R_' not in bot.current_symbol: return jsonify({'error':'Híbrido disponível apenas para índices R_'}), 400
        sig,conf_a = bot.calculate_signal(); da=analyzer.get_analysis(); dr=da.get('recommended_action'); dc=da.get('confidence',0)
        ...
        ok = client.place_trade('CALL' if action=='BUY' else 'PUT', amt, is_digit=False)
        if ok: credit_affiliate_commission(session.get('user_email'), amt); return jsonify({'status':'ok','message':msg,'confidence':comb})
...
def api_trade_manual():
    try:
        d=request.json; action=d.get('action'); amt=float(d.get('amount',0.35))
        client=get_user_session(session['user_id'])['client']
        if not client or not client.authorized: return jsonify({'error':'Não conectado'}), 400
        ok = client.place_trade('CALL' if action=='BUY' else 'PUT', amt, is_digit=False)
...
def api_clear_history():
    try: get_user_session(session['user_id'])['trading_bot'].reset_stats(); return jsonify({'status':'ok'})
    except Exception as e: return jsonify({'error':str(e)}), 500

@app.route('/api/report')
@require_auth
def api_report():
    try: return jsonify(get_user_session(session['user_id'])['trading_bot'].get_trade_report())
    except Exception as e: return jsonify({'error':str(e)}), 500

@app.route('/api/pause', methods=['POST'])
@require_auth
def api_pause():
    d=request.json; p=d.get('paused',True); bot=get_user_session(session['user_id'])['trading_bot']
    bot.pause() if p else bot.resume(); return jsonify({'paused':p})

@app.route('/api/martingale/status', methods=['GET'])
@require_auth
def api_martingale_status():
    try: return jsonify(get_user_session(session['user_id'])['trading_bot'].get_martingale_status())
    except Exception as e: return jsonify({'error':str(e)}), 500
...
def api_martingale_apply():
    try:
        ...
        ok,res = get_user_session(session['user_id'])['trading_bot'].apply_martingale_after_loss(la)
...
def api_martingale_reset():
    try: get_user_session(session['user_id'])['trading_bot'].reset_martingale(); return jsonify({'status':'ok'})
    except Exception as e: return jsonify({'error':str(e)}), 500
...
def api_affiliate_earnings():
    email=session.get('user_email'); user=users.get(email)
    if not user: return jsonify({'error':'Utilizador não encontrado'}), 404
    return jsonify({'earnings':user.get('affiliate_earnings',0),'referral_link':user.get('referral_link_code',''),
                    'referred_count':len(user.get('referred_users',[])),'referred_list':user.get('referred_users',[])})
...
def api_withdraw():
    try:
        ...
        client=get_user_session(session['user_id'])['client']
        if not client or not client.authorized: return jsonify({'error':'Não conectado'}), 400
        if amt>client.balance: return jsonify({'error':'Saldo insuficiente'}), 400
